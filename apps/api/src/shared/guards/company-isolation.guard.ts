...
export class CompanyIsolationGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    if (routeCompanyId && routeCompanyId !== user.companyId) {
      throw new ForbiddenException("Akses ditolak: Anda tidak dapat mengakses data perusahaan lain");
    }
    ...
    if (bodyCompanyId && bodyCompanyId !== user.companyId) {
      throw new ForbiddenException("Akses ditolak: Anda tidak dapat membuat/mengubah data untuk perusahaan lain");
    }
